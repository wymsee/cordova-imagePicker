...
- (BOOL)shouldSelectAsset:(ELCAsset *)asset previousCount:(NSUInteger)previousCount
{
    ...
    if (!shouldSelect) {
        NSString *title = [NSString stringWithFormat:NSLocalizedString(@"사진선택 초과", nil), self.maximumImagesCount];
        NSString *message = [NSString stringWithFormat:NSLocalizedString(@"사진은 최대 6장까지 등록가능합니다.", nil)];
